...
CREATE TABLE IF NOT EXISTS `taxes`.`report_body` (
  ...
  CONSTRAINT `error_id`
    ...
    REFERENCES `taxes`.`report_errors` (`report_errors_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `first_group_id`
    FOREIGN KEY (`first_group_id`)
    REFERENCES `taxes`.`first_group` (`first_group_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  ...
  CONSTRAINT `info_id`
    FOREIGN KEY (`info_id`)
    REFERENCES `taxes`.`report_info` (`report_info_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `second_group_id`
    FOREIGN KEY (`second_group_id`)
    REFERENCES `taxes`.`second_group` (`second_group_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `third_group_id`
    FOREIGN KEY (`third_group_id`)
    REFERENCES `taxes`.`third_group` (`third_group_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
AUTO_INCREMENT = 26
...
CREATE TABLE IF NOT EXISTS `taxes`.`report` (
  ...
  CONSTRAINT `inspector_id`
    ...
    REFERENCES `taxes`.`inspector` (`account_id`),
  CONSTRAINT `user_id`
    FOREIGN KEY (`user_id`)
    REFERENCES `taxes`.`user` (`account_id`))
ENGINE = InnoDB
AUTO_INCREMENT = 30
DEFAULT CHARACTER SET = utf8;
